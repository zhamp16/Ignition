...
sourceLinePath = 'Acushnet\Ball Plant 03\OPP\Line 08' targetLinePath = 'Acushnet\Ball Plant 03\ASB\ASB_02'
...
targetEquipmentName = targetLinePath.split('\')[-1] print "Target equipment name: " + targetEquipmentName

try: sourceOpDef = system.mes.loadMESObject(sourceOpName, 'OperationsDefinition')
...
# Get changeover end trigger (this has duration and auto-end)
...
if len(endTriggers) > 0:
    ...
    changeoverSettings['auto'] = endTrigger.getAuto()
...
except Exception as e: print "Error reading source settings: " + str(e) import traceback traceback.print_exc() raise Exception("Failed to read source settings: " + str(e))

Get the target equipment MES object for setting infeed/outfeed references
try: targetEquipmentLink = system.mes.getMESObjectLinkByEquipmentPath(targetLinePath) targetEquipmentUUID = targetEquipmentLink.getMESObjectUUID() targetEquipmentType = targetEquipmentLink.getMESObjectTypeName() except Exception as e: print "Could not get target equipment link: " + str(e) targetEquipmentUUID = None targetEquipmentType = None
...
for opSeg in targetOperList: # Set TrackProgressBy on OperationsDefinition if opSeg.getMESObjectTypeName() == 'OperationsDefinition': opSeg.setTrackProgressBy(settings['trackProgressBy']) print "\nSet TrackProgressBy: " + str(settings['trackProgressBy'])
...
if opSeg.getMESObjectTypeName() == 'OperationsSegment':
    if '_CO' in opSeg.getName():
        ...
        # Apply changeover end trigger settings (duration and auto-end)
        ...
        if len(targetEndTriggers) > 0 and changeoverSettings:
            ...
            if 'auto' in changeoverSettings:
                targetEndTrigger.setAuto(changeoverSettings['auto'])
...
print "\n========== Saving Changes ==========" system.mes.saveMESObjects(targetOperList)

print "\nMaterial '%s' settings copied successfully!" % materialName print " Source: %s" % sourceLinePath print " Target: %s" % targetLinePath print " - Track Progress By: Copied" print " - Idle Settings: Copied" print " - Changeover Duration: %s minute(s)" % str(changeoverSettings.get('fixedDuration', 'N/A')) print " - Auto End Changeover: %s" % str(changeoverSettings.get('auto', 'N/A')) print " - Production Settings: Copied" print " - Infeed/Outfeed Equipment: Set to " + targetEquipmentName